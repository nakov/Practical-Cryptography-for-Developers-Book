...
For curves with **cofactor** = **1**, all possible EC points over the curve, that can be generated from the generator **G** (when G and n are carefully selected), are all the **n** curve points (including the special point _infinity_). The number **n** is called "**order of the curve**".
